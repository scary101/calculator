...
do
{
    ...
    while (action != "9")
    {
        ...
        if ((a1 & b1) == true)
        {
            a = Convert.ToSingle(user_input_numA);
            b = Convert.ToSingle(user_input_numB);
        }
        else
        {
            Console.Clear();
            Console.WriteLine("ERROR, ВВЕДЕНО НЕ ЧИСЛОВОЕ ЗНАЧЕНИЕ. ПЕРЕЗАПУСК ПРОГРАММЫ!");  
            break;
        }
        ...
        else
        {
            ...
            Console.Clear();
            Console.WriteLine("ERROR, ВВЕДЕНО НЕ КОРРЕКТНОЕ  ДЕЙСТВИЕ. ПЕРЕЗАПУСК ПРОГРАММЫ!");
            break;
        }
